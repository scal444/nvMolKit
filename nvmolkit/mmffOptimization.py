...
def MMFFOptimizeMoleculesConfs(
    ...
    def _normalize_properties(value):
        ...
        if isinstance(value, Sequence) and not hasattr(value, "SetMMFFVariant"):
            if len(value) != len(molecules):
                raise ValueError(
                    f"Expected {len(molecules)} MMFFMolProperties objects, got {len(value)}"
                )
            return [
                default_rdkit_mmff_properties(mol) if props is None else props
                for mol, props in zip(molecules, value)
            ]
        return [value for _ in molecules]
    ...
    interfrag_flags = _normalize_scalar_or_list(
    ...
    native_properties = [
        ...
        for props, threshold, ignore_interfrag in zip(
            properties_list, thresholds, interfrag_flags
        )
    ]
