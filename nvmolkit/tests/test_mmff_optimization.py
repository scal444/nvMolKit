...
def test_mmff_optimization_per_molecule_properties_vs_rdkit(mmff_test_mols):
    ...
    properties = [
        make_rdkit_mmff_properties(mol, settings)
        for mol, settings in zip(nvmolkit_mols, property_settings)
    ]
...
def test_mmff_optimization_per_molecule_thresholds_and_interfrag_vs_rdkit():
    ...
    nvmolkit_mols = create_hard_copy_mols(mols)
    properties = [
        make_rdkit_mmff_properties(mol)
        for mol in nvmolkit_mols
    ]
    ...
    rdkit_energies = [
        ...
        for mol, threshold, ignore_interfrag in zip(
            rdkit_mols, non_bonded_thresholds, ignore_interfrag_interactions
        )
    ]
    nvmolkit_energies = nvmolkit_mmff.MMFFOptimizeMoleculesConfs(
        nvmolkit_mols,
