...
# fourteen physical CPU cores. This runner intentionally has no multi-GPU mode
# or molecule-size scans.
...
Options:
  ...
  --no-autotune        Use fixed CPU-bounded defaults instead of autotuning nvMolKit
...
The suite runs Butina clustering, conformer RMSD, ETKDG, all MMFF/UFF x
BFGS/FIRE combinations, all supported substructure sets, and TFD. The pyperf
cross-similarity microbenchmark is intentionally excluded.
EOF
}
...
if [ "$SKIP_NVMOLKIT" = "0" ] && [ "$CPUS" -lt 2 ]; then
  echo "Error: nvMolKit benchmarks require at least 2 CPUs (one GPU worker and one preprocessing worker)" >&2
  exit 2
fi

NVMOLKIT_PREP_THREADS="$CPUS"
SUBSTRUCT_WORKERS=1
SUBSTRUCT_PREP_THREADS=$(( CPUS - SUBSTRUCT_WORKERS ))
...
for module in nvtx numpy pandas rdkit torch tqdm; do
...
run_bench() {
  ...
  if [ "$code" -eq 0 ] && [ ! -s "$result_path" ]; then
    echo "Error: benchmark exited successfully without writing a non-empty result: $result_path" >> "$log_path"
    code=1
  fi
  if [ "$code" -eq 0 ]; then
    status=ok
...
if [ "$SKIP_RDKIT" = "1" ]; then
  ...
  CONFORMER_RMSD_MODE_FLAGS=(--no-rdkit --no_validate)
  ETKDG_MODE_FLAGS=(--no-rdkit --no_validate)
  FF_MODE_FLAGS=(--no-rdkit --no_validate)
  SUBSTRUCT_MODE_FLAGS=(--no-rdkit --no_validate)
  TFD_MODE_FLAGS=(--no-rdkit)
elif [ "$SKIP_NVMOLKIT" = "1" ]; then
  BUTINA_MODE_FLAGS=(--no-nvmolkit --no-fused)
  CONFORMER_RMSD_MODE_FLAGS=(--no-nvmolkit --no_validate)
  ETKDG_MODE_FLAGS=(--no-nvmolkit --no_validate)
  FF_MODE_FLAGS=(--no-nvmolkit --no_validate)
  SUBSTRUCT_MODE_FLAGS=(--no-nvmolkit --no_validate)
  TFD_MODE_FLAGS=(--no-nvmolkit)
fi
...
run_bench etkdg "$RESULT_DIR/etkdg.csv" \
  ...
  --num_gpus 1 \
  --prep_threads "$NVMOLKIT_PREP_THREADS" \
  --rdkit_threads "$CPUS" \
  --rdkit_max_seconds "$RDKIT_MAX_SECONDS" \
  "${ETKDG_AUTOTUNE_FLAGS[@]}" \
  "${AUTOTUNE_SAVE_FLAGS[@]}" \
...
for row in "${SUBSTRUCT_ROWS[@]}"; do
  ...
  run_bench "$bench_name" "$RESULT_DIR/${bench_name}.csv" \
    ...
    --mode "$mode" \
    --num_gpus 1 \
    --workers "$SUBSTRUCT_WORKERS" \
    --prep_threads "$SUBSTRUCT_PREP_THREADS" \
    --rdkit_threads "$CPUS" \
    --rdkit_max_seconds "$RDKIT_MAX_SECONDS" \
    "${SUBSTRUCT_AUTOTUNE_FLAGS[@]}" \
    "${AUTOTUNE_SAVE_FLAGS[@]}" \
