...
def single_minimization(scheme, half_step, mass_weighting, n_min_for_increase=5, dt_init=0.001, dt_increment=1.1, alpha_init=0.25, max_step=0.0, dt_max_factor=10.0):
    fire_debug = []
    suppl = Chem.SDMolSupplier('/home/kboyd/data/fire/inital_confs.sdf', removeHs = False)
    mol = suppl[0]
    MMFFOptimizeMoleculesConfs([mol], maxIters=500, optimizer_backend="FIRE",
...
for scan_param_tuple in tqdm.tqdm(scan_param_tuples):
    ...
    if fire_debug[0][0]['energies'][-1] < 10 ** 4:
        plt.plot(fire_debug[0][0]['energies'], label='Semi-Implicit Euler + half step + dt_init = ' + str(scan_param_tuple[0]) + ' + dt_increment = ' + str(scan_param_tuple[1]) + ' + n_min_for_increase = ' + str(scan_param_tuple[2]) + ' + max_step = ' + str(scan_param_tuple[3]) + ' + dt_max_factor = ' + str(scan_param_tuple[4]))
