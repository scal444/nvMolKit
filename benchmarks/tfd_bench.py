    ...
    for num_confs in conformer_counts:
        ...
        for num_mols in mol_counts:
            ...
            if not skip_rdkit:
                ...
                rdkit_molecules_processed = timing.progress
                rdkit_time, rdkit_std = timing.mean_ms, timing.std_ms
                result["rdkit_time_ms"] = rdkit_time
                result["rdkit_std_ms"] = rdkit_std
                result["rdkit_molecules_processed"] = rdkit_molecules_processed
                suffix = f" [truncated at {rdkit_molecules_processed}/{len(mols)} mols]" if timing.truncated else ""
            ...
            if not skip_nvmolkit:
                ...
                for key, label in [
                    ...
                    if (
                        result.get("rdkit_time_ms")
                        and result.get("rdkit_molecules_processed") == num_mols
                        and result.get(key)
                    ):
