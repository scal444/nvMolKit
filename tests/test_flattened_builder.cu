...
TEST_F(FlattenedBuilderTestFixture, TestETKDGFlattenedBuilderMol1) {
  ...
  // Note: weights are no longer stored in the structure, they are passed as parameters
  ...
  // EXPECT_EQ(ffParams.chiralTerms.weight[0], 1);  // Weight no longer stored

  // FourthDim
  const int wantNumFourthDimTerms = 9;
  EXPECT_EQ(ffParams.fourthTerms.idx.size(), wantNumFourthDimTerms);
  // EXPECT_EQ(ffParams.fourthTerms.weight.size(), wantNumFourthDimTerms);  // Weight no longer stored

  EXPECT_EQ(ffParams.fourthTerms.idx[0], 0);
  // EXPECT_EQ(ffParams.fourthTerms.weight[0], 0.1);  // Weight no longer stored
}
