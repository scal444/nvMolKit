...
# in a fresh git worktree, with its own conan2 cache so concurrent invocations
# for the same python don't race on shared boost recipe refs.
#
# Usage: build_one_wheel.sh <rdkit_version> <python_version>
#
# Inputs read from environment (set by build_full_matrix.sh):
#   REPO              : repo root (where the .git dir lives)
#   WHEELHOUSE        : output dir; wheel ends up at <wheelhouse>/rdkit<X>/py<Y>/
#   LOG_DIR           : per-pair log dir
#   WORKTREE_ROOT     : where to create the throwaway worktree
...
git -C "$REPO" worktree add --detach "$wt" HEAD > "$logFile" 2>&1
