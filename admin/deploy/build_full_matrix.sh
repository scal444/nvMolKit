...
#
# Each job runs in its own git worktree (so cibuildwheel's per-build _skbuild/
# and CMakeCache.txt don't collide across parallel runs) and writes its wheel
# under wheelhouse/rdkit<RDKIT>/py<PY>/.
#
# Usage:
