...
def write_index(path: Path, links: list[Link]) -> None:
    ...
    for link in links:
        lines.append(
            f'<a href="{escape(link.href, quote=True)}">{escape(link.text)}</a><br>'
        )
...
def main() -> int:
    ...
    print(
        f"Merged {len(index_paths)} generated simple-index page(s) into "
        f"{destination_root}; {changed} page(s) changed."
    )
    return 0
