"""Add a PEP 440 local version segment to an nvMolKit wheel.

Reads a wheel from disk, rewrites the Version field in METADATA, the dist-info
directory name, and the auditwheel SBOM, and emits a new wheel with the
updated filename. RECORD is recomputed from scratch so all hashes stay correct.

Usage:
    python retag_wheel.py <input.whl> <output_dir> <local_version>

Example:
    python retag_wheel.py wheelhouse/rdkit2025.9.6/py3.11/nvmolkit-0.5.0-cp311-...whl \
        wheelhouse/variants/rdkit2025.9.6 rdkit2025.9.6
"""
...
def patch_sbom(sbom_bytes: bytes, old_version: str, new_version: str,
               old_filename: str, new_filename: str) -> bytes:
...
def retag(input_path: str, output_dir: str, local_version: str) -> str:
    ...
    if parts[0] != distribution or parts[1] != old_version:
        raise RuntimeError(
            f"filename {input_basename!r} does not match dist-info {old_dist_info!r}"
        )
    parts[1] = new_version
    ...
    entries = renamed
    rename_map = {old_name: (new_dist_info + old_name[len(old_dist_info):])
