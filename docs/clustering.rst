...
:mod:`nvmolkit.clustering` provides Butina and Leader. Each algorithm has two
forms:
...
``O(N^2)``. Leader accepts ``float32`` or ``float64`` matrices; Butina requires
``float64``.

The fused form takes packed fingerprints and a similarity metric, and
...
``cutoff=0.3``. Leader compares distances in single precision, so a distance
within rounding of the cutoff can be classified differently than by a
