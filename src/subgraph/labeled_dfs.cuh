...
// Per-pair lookup tables and candidates oracle for the warp DFS.
//
...
//   - buildCandidates, the oracle handed to dfsFromRoots.
//
...
#ifndef NVMOLKIT_SUBGRAPH_LABELED_DFS_CUH
#define NVMOLKIT_SUBGRAPH_LABELED_DFS_CUH
...
struct QueryBondPlan {
  ...
  /// Bitset over depths: bit d set iff depth d's only back edge goes to depth
  /// d-1 and resolves through targetAdjacency. Such a depth needs no back-edge
  /// lookup, since the caller just chose depth d-1's target atom.
...
#endif  // NVMOLKIT_SUBGRAPH_LABELED_DFS_CUH
