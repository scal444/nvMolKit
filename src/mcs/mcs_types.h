...
struct MCSParameters {
  ...
  /// Permit RDKit CPU fallback when connectedOnly or maximizeBonds is false,
  /// atomCompare is AnyHeavyAtom, either molecule exceeds 128 atoms or bonds,
  /// or the GPU search reports capacity overflow. CUDA remains mandatory.
  bool                     allowRDKitFallback   = true;
