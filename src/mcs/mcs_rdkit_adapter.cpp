...
bool shouldFallbackToRDKit(const RDKit::ROMol&  molA,
  ...
  // Do NOT add a blockSize gate here: the generated single-occupancy size at
  // tier-128 is handled on the GPU via global substructure scratch
  // (params.scratchLocation), so it does not need to fall back to RDKit.
  return false;
}
