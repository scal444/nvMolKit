...
#include "src/mcs/fmcs_cuda/kf_rewrite/fmcs_kernel_opt.cuh"
...
// ===========================================================================
template <int T>
__global__ __launch_bounds__(kBlockThreads) void fmcsOptKernel(
  const mcs::fmcs::DevicePerPairInput* __restrict__ pairs,
  mcs::fmcs::DeviceMCSResult<T, T>* __restrict__ results,
  unsigned char* __restrict__ queueAll,
  size_t queueStrideBytes,
  int numPairs) {
  using W = typename WordSel<T>::type;
  ...
  const mcs::fmcs::DevicePerPairInput& input = pairs[pairIdx];
  PairPtrs       P;
  P.qNA  = input.queryNumAtoms;
  P.qNB  = input.queryNumBonds;
  P.tNA  = input.targetNumAtoms;
  P.tNB  = input.targetNumBonds;
  P.qRow = input.queryRowOffsets;
  P.qCol = input.queryColIndices;
  P.qBnd = input.queryBondIndices;
  P.qEp  = input.queryBondEndpoints;
  P.tRow = input.targetRowOffsets;
  P.tCol = input.targetColIndices;
  P.tBnd = input.targetBondIndices;
  P.tEp  = input.targetBondEndpoints;
  P.aTab = input.tables.atoms.data;
  P.aWPR = input.tables.atoms.wordsPerRow;
  P.bTab = input.tables.bonds.data;
  P.bWPR = input.tables.bonds.wordsPerRow;

  QS<T>* qbase = reinterpret_cast<QS<T>*>(queueAll + static_cast<size_t>(pairIdx) * queueStrideBytes);
  ...
  auto& result = results[pairIdx];
  if (tid == 0) {
    result.numCommonVertices = nA;
    result.numCommonEdges    = nB;
    result.timedOut          = false;
    result.overflowed        = S.overflowed != 0;
  }
  for (int a = tid; a < T; a += kBlockThreads) {
    if ((bAtom >> a) & 1) {
      const W   lowm = (static_cast<W>(1) << a) - 1;
      const int idx  = popcW(bAtom & lowm);
      result.mappingA[idx] = static_cast<uint8_t>(a);
      result.mappingB[idx] = S.best.match.tAtom[a];
    }
    if ((bBond >> a) & 1) {
      const W   lowm = (static_cast<W>(1) << a) - 1;
      const int idx  = popcW(bBond & lowm);
      result.bondMapA[idx] = static_cast<uint8_t>(a);
      result.bondMapB[idx] = S.best.match.tBond[a];
    }
  }
}
