    ...
    if (scanQueryAdjacency) {
      ...
      for (int wordIdx = 0; wordIdx < kBondWords; ++wordIdx) {
        ...
        while (remaining != 0) {
          ...
          if constexpr (sizeof(BondWord) == 4) {
            bitPosInWord =
                __ffs(static_cast<unsigned int>(remaining)) - 1;
          } else {
            bitPosInWord =
...
__device__ __forceinline__ int countCandidateTargetAtomsCooperative(
  ...
  for (int targetBase = 0;
    ...
    if (compatible) {
      compatible =
          scratch.targetDegree[targetAtomIdx] >= scratch.seedDegree[queryAtomIdx] &&
  ...
  return group.shfl(candidateCount, 0);
}
  ...
  for (int orderPos = 0; orderPos < numSeedAtoms; ++orderPos) {
    ...
    if (laneRank == 0) {
      if (bestAtom < 0) {
        ...
        incrementMappedSeedNeighborCountsWithinThread(
            seed, queryTopology, scratch, bestAtom);
      }
    }
    prepareOk = group.shfl(prepareOk, 0);
    if (!prepareOk) break;
...
__device__ __forceinline__ bool matchSeedSubstructureCooperative(
  ...
  for (int depth = 1; depth < numSeedAtoms; ++depth) {
    ...
    int hoistedNeighborOrderPos = -1;
    bool hoistedScanAdjacency = false;
    if constexpr (HoistNeighborOrder) {
        ...
        for (int partialBase = 0;
          ...
          for (int targetScanIdx = targetScanBegin + sublaneRank;
            ...
            tryExtendSubstructurePartialWithinThread(
                seed, queryTopology, targetTopology, tables, scratch,
                partial, depth, numSeedAtoms, queryAtomIdx, targetAtomIdx,
                stride, nextPartials, effectiveCapacity);
          }
        }
      } else {
