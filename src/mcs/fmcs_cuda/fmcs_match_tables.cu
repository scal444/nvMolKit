...
void checkCuda(cudaError_t err, const char* context) {
  if (err != cudaSuccess) {
    throw std::runtime_error(std::string("fMCS match-tables CUDA error at ")
                             + context + ": "
...
std::vector<PairMatchTablesDevice> uploadPairMatchTables(
  ...
  for (size_t i = 0; i < host.size(); ++i) {
    ...
    if (!ph.atoms.data.empty()) {
      ...
      checkCuda(cudaMemcpyAsync(dst, ph.atoms.data.data(),
    ...
    if (!ph.bonds.data.empty()) {
      uint32_t* dst = base + cursor;
      checkCuda(cudaMemcpyAsync(dst, ph.bonds.data.data(),
  ...
  if (outDeviceBuffer) *outDeviceBuffer = buf;
  if (outDeviceBufferBytes) *outDeviceBufferBytes = totalWords * sizeof(uint32_t);
  return out;
}
