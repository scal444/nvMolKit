...
void launchFmcsKernelSpecialization(const DevicePerPairInput* pairs,
                                    ...
                                    cudaStream_t stream) {
  configureFmcsKernelSharedMem<blockThreads, maxAtoms, maxBonds, Scratch,
...
void launchFmcsKernelSelected(const DevicePerPairInput* pairs,
  ...
  if (collectTimings && collectStats) {
    if constexpr (nvMolKit::kMCSCollectTimingsEnabled &&
      ...
      launchFmcsKernelSpecialization<blockThreads, maxAtoms, maxBonds, Scratch, true, true>(
          pairs, results, queueStorage, substructureStorage, scratchStorage,
          elapsedClocks, statsOut, queueCapacity, substructurePartialCapacity,
    ...
    if constexpr (nvMolKit::kMCSCollectTimingsEnabled) {
      launchFmcsKernelSpecialization<blockThreads, maxAtoms, maxBonds, Scratch, true, false>(
          pairs, results, queueStorage, substructureStorage, scratchStorage,
          elapsedClocks, timingStatsOut, queueCapacity,
          substructurePartialCapacity, numPairs, timeoutClocks, stream);
      return;
    }
    ...
    if constexpr (nvMolKit::kMCSCollectStatsEnabled) {
      launchFmcsKernelSpecialization<blockThreads, maxAtoms, maxBonds, Scratch, false, true>(
          pairs, results, queueStorage, substructureStorage, scratchStorage,
          nullptr, statsOut, queueCapacity, substructurePartialCapacity,
          numPairs, timeoutClocks, stream);
      return;
    }
  } else {
    launchFmcsKernelSpecialization<blockThreads, maxAtoms, maxBonds, Scratch, false, false>(
        pairs, results, queueStorage, substructureStorage, scratchStorage,
        nullptr, nullptr, queueCapacity, substructurePartialCapacity, numPairs,
...
void configureFmcsKernelMaxDynamicSharedMem(const void* kernelFunc,
                                            std::size_t bytes) {
  const cudaError_t err =
      cudaFuncSetAttribute(kernelFunc,
                           cudaFuncAttributeMaxDynamicSharedMemorySize,
                           static_cast<int>(bytes));
  if (err != cudaSuccess) {
    throw std::runtime_error(
        std::string("fMCS CUDA error configuring max dynamic shared memory: ") +
        cudaGetErrorString(err));
  }
}
...
std::size_t fmcsKernelStaticSharedBytes(FmcsScratchLocation scratchLocation) {
  ...
  if (scratchLocation == FmcsScratchLocation::Global) {
    err = cudaFuncGetAttributes(
        &attr,
        fmcsKernel<maxAtoms, maxBonds, blockThreads, false, false,
                   FmcsScratchLocation::Global>);
  } else {
    if constexpr (blockThreads == 512 && maxAtoms == 128) {
      throw std::invalid_argument(
          "fMCS blockSize 512 at tier-128 has no shared-scratch kernel");
    } else {
      err = cudaFuncGetAttributes(
          &attr,
          fmcsKernel<maxAtoms, maxBonds, blockThreads, false, false,
  ...
  if (err != cudaSuccess) {
    throw std::runtime_error(
        std::string("fMCS CUDA error querying kernel attributes: ") +
        cudaGetErrorString(err));
  }
...
void launchFmcsKernel128(const DevicePerPairInput* pairs,
  ...
  if (scratchLocation == FmcsScratchLocation::Global) {
    ...
    if constexpr (maxAtoms == 128) {
      launchFmcsKernelSelected<128, maxAtoms, maxBonds, FmcsScratchLocation::Global>(
          pairs, results, queueStorage, substructureStorage, scratchStorage,
          elapsedClocks, timingStatsOut, statsOut, queueCapacity,
          substructurePartialCapacity, numPairs, timeoutClocks, stream);
      return;
    }
    throw std::invalid_argument(
  ...
  launchFmcsKernelSelected<128, maxAtoms, maxBonds, FmcsScratchLocation::Shared>(
...
void launchFmcsKernel512(const DevicePerPairInput* pairs,
                         ...
                         cudaStream_t stream) {
  if (scratchLocation == FmcsScratchLocation::Global) {
    launchFmcsKernelSelected<512, maxAtoms, maxBonds, FmcsScratchLocation::Global>(
        pairs, results, queueStorage, substructureStorage, scratchStorage,
        elapsedClocks, timingStatsOut, statsOut, queueCapacity,
        substructurePartialCapacity, numPairs, timeoutClocks, stream);
    return;
  ...
  if constexpr (maxAtoms == 128) {
    ...
    launchFmcsKernelSelected<512, maxAtoms, maxBonds, FmcsScratchLocation::Shared>(
        pairs, results, queueStorage, substructureStorage, nullptr,
        elapsedClocks, timingStatsOut, statsOut, queueCapacity,
        substructurePartialCapacity, numPairs, timeoutClocks, stream);
  }
...
template void launchFmcsKernel128<16, 16>(
    ...
    std::uint8_t*, void*, FmcsScratchLocation, unsigned long long*,
    ExecutionStats*, ExecutionStats*, int, int, int, unsigned long long,
    cudaStream_t);
...
template void launchFmcsKernel512<64, 64>(
    ...
    ExecutionStats*, ExecutionStats*, int, int, int, unsigned long long,
    cudaStream_t);
template void launchFmcsKernel512<128, 128>(
    const DevicePerPairInput*, FmcsDeviceResult<128, 128>*, void*,
    std::uint8_t*, void*, FmcsScratchLocation, unsigned long long*,
    ExecutionStats*, ExecutionStats*, int, int, int, unsigned long long,
    cudaStream_t);
