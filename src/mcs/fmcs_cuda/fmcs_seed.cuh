...
/// @c seedAtomIdx records the already-in-seed endpoint for atom-adding bonds
/// so the child can keep that branch point in its next frontier.
...
template <int maxAtoms, int maxBonds> struct Seed {
  ...
  /// Snapshot of @c numAtoms at the start of the most recent grow
  /// step.  Kept alongside @c lastAddedAtoms purely as a count for
  /// callers that need to know how many atoms were added in this step
  /// without iterating the bitset.
  uint16_t lastAddedAtomsBegin = 0;
...
/// grow step.
template <int maxAtoms, int maxBonds>
__device__ __forceinline__ void seedMarkLastAddedAtomWithinThread(Seed<maxAtoms, maxBonds>& seed, const int atomIdx) {
  using AtomWord              = typename Seed<maxAtoms, maxBonds>::atom_word_type;
  constexpr int  kBitsPerWord = Seed<maxAtoms, maxBonds>::kAtomBitsPerWord;
  const int      wordIdx      = atomIdx / kBitsPerWord;
  const AtomWord mask         = static_cast<AtomWord>(1) << (atomIdx % kBitsPerWord);
...
__device__ __forceinline__ void seedBeginGrowStepWithinThread(Seed<maxAtoms, maxBonds>& seed) {
  ...
  seed.lastAddedAtomsBegin = seed.numAtoms;
}
...
__device__ __forceinline__ void seedComputeRemainingSizeWithinThread(Seed<maxAtoms, maxBonds>& seed,
  ...
  for (int i = 0; i < Seed<maxAtoms, maxBonds>::kBondWords; ++i) {
    ...
    if constexpr (sizeof(BondWord) == 4) {
      ...
      excludedBondCount += __popcll(static_cast<unsigned long long>(word));
    }
  }
  seed.remainingAtoms = static_cast<uint16_t>(queryTopology.numAtoms - seed.numAtoms);
  seed.remainingBonds = static_cast<uint16_t>(queryTopology.numBonds - excludedBondCount);
}
