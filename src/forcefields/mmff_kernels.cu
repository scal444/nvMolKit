...
__global__ void positionConstraintEnergyKernel(const int     numConstraints,
  ...
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < numConstraints) {
    const double energy = positionConstraintEnergy(
      pos, idxs[idx], refXs[idx], refYs[idx], refZs[idx], maxDispls[idx], forceConstants[idx]);
...
__global__ void positionConstraintGradKernel(const int     numConstraints,
  ...
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < numConstraints) {
    positionConstraintGrad(pos, idxs[idx], refXs[idx], refYs[idx], refZs[idx], maxDispls[idx], forceConstants[idx], grad);
  }
}
...
__global__ void angleConstraintEnergyKernel(const int     numConstraints,
  ...
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < numConstraints) {
    const double energy = angleConstraintEnergy(
      pos, idx1s[idx], idx2s[idx], idx3s[idx], minAngleDegs[idx], maxAngleDegs[idx], forceConstants[idx]);
    const int batchIdx  = atomBatchMap[idx1s[idx]];
    const int outputIdx = getEnergyAccumulatorIndex(idx, batchIdx, energyBufferStarts, termBatchStarts);
    energyBuffer[outputIdx] += energy;
  }
...
__global__ void angleConstraintGradKernel(const int     numConstraints,
  ...
  if (idx < numConstraints) {
    angleConstraintGrad(
      pos, idx1s[idx], idx2s[idx], idx3s[idx], minAngleDegs[idx], maxAngleDegs[idx], forceConstants[idx], grad);
  }
}
...
cudaError_t launchDistanceConstraintEnergyKernel(const int     numConstraints,
  ...
  distanceConstraintEnergyKernel<<<numBlocks, blockSize, 0, stream>>>(
    numConstraints, idx1, idx2, minLen, maxLen, forceConstant, pos, energyBuffer, energyBufferStarts, atomBatchMap, termBatchStarts);
  return cudaGetLastError();
}
...
cudaError_t launchDistanceConstraintGradientKernel(const int     numConstraints,
  ...
  constexpr int blockSize = 256;
  const int     numBlocks = (numConstraints + blockSize - 1) / blockSize;
  distanceConstraintGradKernel<<<numBlocks, blockSize, 0, stream>>>(
    numConstraints, idx1, idx2, minLen, maxLen, forceConstant, pos, grad);
  return cudaGetLastError();
}
...
cudaError_t launchPositionConstraintEnergyKernel(const int     numConstraints,
  ...
  constexpr int blockSize = 256;
  const int     numBlocks = (numConstraints + blockSize - 1) / blockSize;
  positionConstraintEnergyKernel<<<numBlocks, blockSize, 0, stream>>>(
    numConstraints, idx, refX, refY, refZ, maxDispl, forceConstant, pos, energyBuffer, energyBufferStarts, atomBatchMap, termBatchStarts);
  return cudaGetLastError();
}
...
cudaError_t launchPositionConstraintGradientKernel(const int     numConstraints,
  ...
  constexpr int blockSize = 256;
  const int     numBlocks = (numConstraints + blockSize - 1) / blockSize;
  positionConstraintGradKernel<<<numBlocks, blockSize, 0, stream>>>(
    numConstraints, idx, refX, refY, refZ, maxDispl, forceConstant, pos, grad);
  return cudaGetLastError();
}
...
cudaError_t launchAngleConstraintEnergyKernel(const int     numConstraints,
  ...
  constexpr int blockSize = 256;
  const int     numBlocks = (numConstraints + blockSize - 1) / blockSize;
  angleConstraintEnergyKernel<<<numBlocks, blockSize, 0, stream>>>(
    numConstraints, idx1, idx2, idx3, minAngleDeg, maxAngleDeg, forceConstant, pos, energyBuffer, energyBufferStarts, atomBatchMap, termBatchStarts);
  return cudaGetLastError();
}
...
cudaError_t launchAngleConstraintGradientKernel(const int     numConstraints,
  ...
  constexpr int blockSize = 256;
  const int     numBlocks = (numConstraints + blockSize - 1) / blockSize;
  angleConstraintGradKernel<<<numBlocks, blockSize, 0, stream>>>(
    numConstraints, idx1, idx2, idx3, minAngleDeg, maxAngleDeg, forceConstant, pos, grad);
  return cudaGetLastError();
}
...
cudaError_t launchTorsionConstraintEnergyKernel(const int     numConstraints,
  ...
  constexpr int blockSize = 256;
  const int     numBlocks = (numConstraints + blockSize - 1) / blockSize;
  torsionConstraintEnergyKernel<<<numBlocks, blockSize, 0, stream>>>(
    numConstraints, idx1, idx2, idx3, idx4, minDihedralDeg, maxDihedralDeg, forceConstant, pos, energyBuffer, energyBufferStarts, atomBatchMap, termBatchStarts);
  return cudaGetLastError();
}
...
cudaError_t launchTorsionConstraintGradientKernel(const int     numConstraints,
  ...
  constexpr int blockSize = 256;
  const int     numBlocks = (numConstraints + blockSize - 1) / blockSize;
  torsionConstraintGradKernel<<<numBlocks, blockSize, 0, stream>>>(
    numConstraints, idx1, idx2, idx3, idx4, minDihedralDeg, maxDihedralDeg, forceConstant, pos, grad);
  return cudaGetLastError();
}
