...
static __device__ __forceinline__ double computeSignedDihedral(const double* pos,
  ...
  t[1][1] /= d[1];
  t[1][2] /= d[1];
  const double cosPhi = clamp(
    t[0][0] * t[1][0] + t[0][1] * t[1][1] + t[0][2] * t[1][2], -1.0, 1.0);
...
static __device__ __forceinline__ void torsionConstraintGrad(const double* pos,
  ...
  const double dedt0[3] = {tmp0[0] / tt0LenSq * prefactor, tmp0[1] / tt0LenSq * prefactor, tmp0[2] / tt0LenSq * prefactor};
  const double dedt1[3] = {tmp1[0] / tt1LenSq * prefactor, tmp1[1] / tt1LenSq * prefactor, tmp1[2] / tt1LenSq * prefactor};
...
#pragma unroll 1
  for (int i = pcStart + tid; i < pcEnd; i += stride) {
    ...
    energy += positionConstraintEnergy(molCoords, localIdx, refXs[i], refYs[i], refZs[i], maxDispls[i], pcForceConstants[i]);
  }
...
#pragma unroll 1
  for (int i = acStart + tid; i < acEnd; i += stride) {
    ...
    energy += angleConstraintEnergy(
      molCoords, localIdx1, localIdx2, localIdx3, minAngleDegs[i], maxAngleDegs[i], acForceConstants[i]);
  }
...
#pragma unroll 1
  for (int i = tcStart + tid; i < tcEnd; i += stride) {
    ...
    energy += torsionConstraintEnergy(
      molCoords, localIdx1, localIdx2, localIdx3, localIdx4, minDihedralDegs[i], maxDihedralDegs[i], tcForceConstants[i]);
  }
...
#pragma unroll 1
  for (int i = acStart + tid; i < acEnd; i += stride) {
    ...
    const int localIdx3 = ac_idx3s[i] - atomStart;
    angleConstraintGrad(
      molCoords, localIdx1, localIdx2, localIdx3, minAngleDegs[i], maxAngleDegs[i], acForceConstants[i], grad);
  }
...
#pragma unroll 1
  for (int i = tcStart + tid; i < tcEnd; i += stride) {
    ...
    const int localIdx3 = tc_idx3s[i] - atomStart;
    const int localIdx4 = tc_idx4s[i] - atomStart;
    torsionConstraintGrad(
      molCoords, localIdx1, localIdx2, localIdx3, localIdx4, minDihedralDegs[i], maxDihedralDegs[i], tcForceConstants[i], grad);
  }
}
