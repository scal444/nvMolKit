...
#ifndef NVMOLKIT_MORGAN_WORKLOAD_INL_H
#define NVMOLKIT_MORGAN_WORKLOAD_INL_H
 ...
 */
inline std::pair<MorganBucket, int> unitToBucketSlice(int                          unitIdx,
                                                      int                          numBatches32,
                                                      int                          numBatches64,
                                                      int                          /*numBatches128*/) {
...
inline int bucketAtomCapacity(MorganBucket bucket) {
  switch (bucket) {
...
void MorganWorkload<fpSize>::preprocess(Inputs&                   inputs,
...
template <int fpSize>
std::unique_ptr<typename MorganWorkload<fpSize>::PerGpuState>
MorganWorkload<fpSize>::makePerGpuState(Inputs& /*inputs*/, int gpuId) {
...
MorganWorkload<fpSize>::makeSlotState(Inputs& inputs, PerGpuState& /*pgs*/, int /*gpuId*/) {
  ...
  auto allocate = [&](MorganGPUBuffersBatch& buffers, int atomCapacity) {
    ...
    switch (atomCapacity) {
      case 32:
        buffers.allSeenNeighborhoods32 = AsyncDeviceVector<FlatBitVect<32>>(
          chunkSize * 32 * static_cast<size_t>(radius + 1),
          stream);
        buffers.allSeenNeighborhoods32.zero();
        break;
      case 64:
        buffers.allSeenNeighborhoods64 = AsyncDeviceVector<FlatBitVect<64>>(
          chunkSize * 64 * static_cast<size_t>(radius + 1),
          stream);
        buffers.allSeenNeighborhoods64.zero();
        break;
      case 128:
        buffers.allSeenNeighborhoods128 = AsyncDeviceVector<FlatBitVect<128>>(
          chunkSize * 128 * static_cast<size_t>(radius + 1),
          stream);
...
#endif  // NVMOLKIT_MORGAN_WORKLOAD_INL_H
